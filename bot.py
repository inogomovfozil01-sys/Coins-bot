...
if not BOT_TOKEN:
    logger.error("BOT_TOKEN не установлен. Установите переменную окружения BOT_TOKEN и перезапустите.")
    sys.exit(1)
...
# безопасные обёртки отправки (чтобы не ломались теги)
...
# ------- База данных -------
DATA_DIR = Path(os.getenv("DATA_DIR", "."))  # можно указать ��апку через переменную окружения
DATA_DIR.mkdir(parents=True, exist_ok=True)

users_db_path = DATA_DIR / "users.db"
games_db_path = DATA_DIR / "bot.db"

# check_same_thread=False для безопасного использования в асинхронном приложении
users_db = sqlite3.connect(users_db_path, check_same_thread=False)
...
games_db = sqlite3.connect(games_db_path, check_same_thread=False)
...
async def pay(message: Message):
    ...
    try:
        ...
        # не критично, пользователь мог отключить сообщения
        pass
